...
function calculate_rhs_taylor(link_p_in, link_p_out, l)
    ...
    weymouth_ct = EMP.weymouth_constant(pressure_data)
...
function generate_case()
    ...
    nodes = [
        ...
        Compressor(4, FixedProfile(1e6), FixedProfile(0), FixedProfile(0), Dict(NG => 1), Dict(NG => 1), FixedProfile(20), FixedProfile(25)),
        # GenAvailability(4, [NG]),
        RefSink(5, FixedProfile(100), Dict(:surplus => FixedProfile(-100), :deficit=> FixedProfile(1e6)), Dict(NG => 1), [MaxPressureData(FixedProfile(180))])
    ]
    links = [
        ...
        CapDirect(34, nodes[3], nodes[4], Linear(), FixedProfile(200), [PressureLinkData(0.24, 200, 130)]),
        CapDirect(45, nodes[4], nodes[5], Linear(), FixedProfile(700), [PressureLinkData(0.24, 200, 130)]),
    ]
...
@testset "Model creation" begin
    @test try
        EMP.create_model(case, model; check_timeprofiles=true)
        true
    catch
        false
    end
...
@testset "Variables Definition" begin
    @testset "Node variables" begin
        ...
        @test sum(nt[1] == 𝒩[1] for nt in keys(m[:potential_out])) == length(𝒯)
        @test sum(nt[1] == 𝒩[2] for nt in keys(m[:potential_out])) == length(𝒯)
        @test sum(nt[1] == 𝒩[3] for nt in keys(m[:potential_out])) == length(𝒯)
    end
    ...
    @testset "Link variables" begin
        ...
        @test sum(nt[1] == ℒ[1] for nt in keys(m[:link_potential_out])) == length(𝒯)
        @test sum(nt[1] == ℒ[2] for nt in keys(m[:link_potential_out])) == length(𝒯)
    end
end
...
@testset "Constraints Definition" begin
    @testset "Nodes Internal Balance Pressure" begin
        ...
        @test all(v -> v == 0, [value(m[:potential_in][nt]) for nt in keys(m[:potential_in]) if nt[1] in 𝒩ˢʳ])
        
        𝒩ˢ = filter(n -> n isa RefSink, 𝒩)
        @test all(v -> v == 0, [value(m[:potential_out][nt]) for nt in keys(m[:potential_out]) if nt[1] in 𝒩ˢ])
        
        𝒩ᵃ = filter(n -> n isa Availability, 𝒩)
        in_val = [value(m[:potential_in][nt]) for nt in keys(m[:potential_in]) if nt[1] in 𝒩ᵃ]
        out_val = [value(m[:potential_out][nt]) for nt in keys(m[:potential_out]) if nt[1] in 𝒩ᵃ]
        @test in_val == out_val

        𝒩ᶜ = filter(n -> n isa Compressor, 𝒩)
        in_val = [value(m[:potential_in][nt]) for nt in keys(m[:potential_in]) if nt[1] in 𝒩ᶜ]
        out_val = [value(m[:potential_out][nt]) for nt in keys(m[:potential_out]) if nt[1] in 𝒩ᶜ]
