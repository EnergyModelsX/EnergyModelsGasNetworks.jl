...
function test_approx(weymouth, pressure_points::Vector, link_potential_in, link_potential_out)
    rhs = Float64[]
    for (p_in, p_out) ∈ pressure_points
        value = sqrt(weymouth) * 
        ((p_in/(sqrt(p_in^2 - p_out^2))) * link_potential_in - (p_out/(sqrt(p_in^2 - p_out^2))) * link_potential_out)
        push!(rhs, value)
    end 
    return minimum(rhs)
end