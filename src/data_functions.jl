...
function EMB.constraints_ext_data(m, n::UnitConversion, 𝒯, 𝒫, modeltype::EMB.EnergyModel, data::FlowToEnergyData)
    ...
    𝒫ⁿ = EMB.inputs(n)
    @constraint(m, [t ∈ 𝒯, p ∈ EMB.outputs(n)], m[:flow_out][n, t, p] == LHV[t] * sum(m[:flow_in][n, t, p] for p in 𝒫ⁿ))
end
...
function resource_lhv(m, n::EMB.Node, 𝒫ˡʰᵛ::Vector{<:EMB.Resource}, data::FlowToEnergyData, 𝒯)
    return @expression(m, [t ∈ 𝒯], sum(get_specific_energy_content(data, p) * m[:proportion_track][n, t, p] for p in 𝒫ˡʰᵛ))
end
