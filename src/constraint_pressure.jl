...
function constraints_flow_pressure(m, l::EMB.Link, 𝒯, 𝒫::Vector{<:ResourceBlend{<:ResourcePotential}}, optimizer)
    
    # Get inputs of `l` that are ResourceBlend
    𝒫ⁿ = [p for p in EMB.inputs(l) if p ∈ 𝒫]
...
"""
function constraints_pwa(m, l::EMB.Link, p_blend::ResourceBlend, p_track::ResourcePotential, 𝒯, plane, pwa::PWAFunc{C1, D1}) where {C1, D1}
    n = l.from
    for t ∈ 𝒯
        PiecewiseAffineApprox.constr(C1, m, m[:link_in][l, t, p_blend], plane, (m[:link_potential_in][l, t, p_blend], m[:link_potential_out][l, t, p_blend], m[:proportion_track][n, t, p_track]))
    end
end