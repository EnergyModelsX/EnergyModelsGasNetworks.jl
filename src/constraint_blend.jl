...
function create_blending_node(m, a::TerminalArea, 𝒜, 𝒞, ℒᵗʳᵃⁿˢ, links, 𝒯)
    ...
    ℒ = Dict(
        ad => EMG.modes(l) for ad ∈ 𝒜ᵃ for l ∈ [EMG.corr_from_to(ad.name, a.name, ℒᵗʳᵃⁿˢ)]
    )
    ...
    @constraint(m, [t ∈ 𝒯, s ∈ 𝒮ᵗᵐ],
        ...
        -
        m[:prop_source][a, s, t] * sum(m[:trans_out][tm, t] for ad ∈ 𝒜ᵃ for tm ∈ ℒ[ad]) ==
        0)
...
function create_blending_node(m, a::PoolingArea, 𝒜, 𝒞, ℒᵗʳᵃⁿˢ, links, 𝒯)
    ...
    @constraint(m, [t ∈ 𝒯, tm ∈ ℒᶠ],
        sum(m[:prop_source][a, s, t] * m[:trans_in][tm, t] for s ∈ 𝒮ᵗᵐ) -
        m[:trans_in][tm, t] == 0)
...
function constraints_tracking(m, a::Area, 𝒜, 𝒞, ℒᵗʳᵃⁿˢ, links, 𝒯)
    ...
    if isnothing(c)
        throw(
            ArgumentError("Trying to build a blending node without a component to track."),
        )
    else
        𝒮ᵗᵐ = track_source(a, links, 𝒜, ℒᵗʳᵃⁿˢ)
        ...
        @constraint(m, [t ∈ 𝒯],
            m[:prop_track][c, a, t] ==
            sum(get_quality(s, c) * m[:prop_source][a, s, t] for s ∈ 𝒮))
...
function constraints_quality(m, a::TerminalArea, 𝒜, ℒᵗʳᵃⁿˢ, links, 𝒯)
    blending_sink =
        [n for n ∈ EMG.getnodesinarea(a, links) if EnergyModelsPooling.is_blending_sink(n)]   # get terminals, one terminal per terinalarea
    ...
    if !isempty(blending_sink)
        ...
        @constraint(m, [t ∈ 𝒯, p ∈ 𝒫ᵘ],
            sum(
                (get_quality(s, p) - get_upper(d, p)) * m[:prop_source][ad, s, t] *
                m[:trans_out][tm, t] for ad ∈ 𝒜ᵃ for s ∈ 𝒮ᵃ[ad] for tm ∈ TM[ad]
            ) <= 0)
        𝒫ˡ = res_lower(d)
        @constraint(m, [t ∈ 𝒯, p ∈ 𝒫ˡ],
            sum(
                (get_quality(s, p) - get_lower(d, p)) * m[:prop_source][ad, s, t] *
                m[:trans_out][tm, t] for ad ∈ 𝒜ᵃ for s ∈ 𝒮ᵃ[ad] for tm ∈ TM[ad]
            ) >= 0)
    else
        throw(
            ArgumentError(
                "Trying to create a TerminalArea with Blending behaviour without a BlendingSink node.",
            ),
        )
    end
end
...
function constraints_energy_content(m, a::TerminalArea, 𝒞, ℒᵗʳᵃⁿˢ, 𝒯)
    ...
    if !isnothing(energy_delivery(a))
        for (idx, t) ∈ enumerate(𝒯)
            @constraint(m,
                m[:energy_content][a, t] >= energy_delivery(a, idx))
            @constraint(m,
                m[:energy_content][a, t] == sum(
                    m[:trans_out][tm_mode, t] * (
                        m[:prop_track][c, tm.from, t] * energy_content(c) +
                        (1-m[:prop_track][c, tm.from, t]) * energy_content(d)
                    ) for tm ∈ ℒᵗᵒ for tm_mode ∈ modes(tm)
                ))
        end
    end
end
